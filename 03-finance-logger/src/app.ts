...
// / <reference path="./namespaces/util.ts" />
...
window.onload = () => {
  ...
  const appName = document.querySelector("#appName") as HTMLHeadingElement;

  const ul = document.querySelector("#finance-list") as HTMLUListElement;
  let list = new ListTemplate(ul);

  ul.innerText = Util.getDetails();
  appName.innerText = Util.getAppName()
  ...
  btnAdd.addEventListener("click", (event: Event) => {
    ...
    if (txtType.value === "invoice") {
      doc = new Invoice(
        txtTitle.value,
        new Date(txtCreateAt.value),
        txtAmount.valueAsNumber
      );
    } else {
      doc = new Payment(
        txtTitle.value,
